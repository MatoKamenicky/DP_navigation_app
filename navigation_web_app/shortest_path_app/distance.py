...
def shortest_path(G, orig_yx, dest_yx, type, orig_edge=None, dest_edge=None):
    '''
    Parameters
    ----------
    G : networkx.MultiDiGraph
    ...
    # routing across multiple edges
        ...
        if type == 'length':
            nx_route = nx_shortest_path(G, orig_edge[0], dest_edge[0], weight='length')
        elif type == 'time':
            nx_route = nx_shortest_path(G, orig_edge[0], dest_edge[0], weight='travel_time')
        else:
            nx_route = nx_shortest_path(G, orig_edge[0], dest_edge[0], weight='length')
        p_o, p_d = Point(orig_yx[::-1]), Point(dest_yx[::-1])
